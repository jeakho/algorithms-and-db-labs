...
export class LinkedList<T = any> {
    ...
    public swap(ind1: number, ind2: number) {
        ...
        const prev1: { next: any } = this.getNode(ind1 - 1) || { next: this.getNode(ind1) };
        const prev2: { next: any } = this.getNode(ind2 - 1) || { next: this.getNode(ind2) };
    ...
    private getNode(ind: number): ListNode<T> | null {
