...
export class TreeControllerComponent implements OnInit, OnDestroy, OnChanges, IBinaryTree<any> {
  ...
  private drawTree() {
    ...
    if (this.binaryTree.isEmpty()) this.printDefault();
  ...
  private printDefault() {
    const { message, fontSize, color } = this.defaultMessageConfig;

    this.graphics.fillText(message, this.graphics.fieldWidth / 2, this.graphics.fieldHeight / 2.4, { fontSize, horizontalAlign: 'center', verticalAlign: 'middle', color });
  }
